...
export class AbiRegistry {
  ...
  private createCustomType(typeDiscriminant: string, json: any): CustomType {
    ...
    throw new errors.ErrTypingSystem(
      `Unknown type discriminant: ${typeDiscriminant}`
    );
  }
  ...
  getStruct(name: string): StructType {
    let result = this.customTypes.find(
      (e) => e.getName() == name && e.hasExactClass(StructType.ClassName)
    );
  ...
  getEnum(name: string): EnumType {
    let result = this.customTypes.find(
      (e) => e.getName() == name && e.hasExactClass(EnumType.ClassName)
    );
  ...
  remapToKnownTypes(): AbiRegistry {
    ...
    for (const iface of this.interfaces) {
      ...
      let newConstructor = iface.constructorDefinition
        ? mapEndpoint(iface.constructorDefinition, mapper)
        : null;
      newInterfaces.push(
        new ContractInterface(iface.name, newConstructor, newEndpoints)
      );
    }
  ...
  private mapCustomTypeDepthFirst(
    ...
    const hasBeenMapped =
      mappedTypes.findIndex((type) => type.getName() == typeToMap.getName()) >=
      0;
    if (hasBeenMapped) {
      return;
    }
